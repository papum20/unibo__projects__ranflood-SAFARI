...
*	`ansible/` : ansible playbooks executed on the remote machines
	...
	*	`files/` : files to transfer to the hosts
		...
		*	`other/` : other files, currently unused (it also includes some files used in our tests, see [initialization](#initialization))
...
*	put the desired files in `ansible/files/`, in particular (as specified in `ansible/variables/variables.sh`) :
	*	in `ansible/files/ransomwares`, put the ransomware(s) you'll use
	*	in `ansible/files/ranflood`, put the `Ranflood` daemon Jar and the client zip
	*	in `ansible/files/filechecker`, put the `filechecker` Jar
	*	the files used in our tests for `Ranflood` and `filechecker` are already provided in `ansible/files/other/` 
